...
export default function Explore(){
  ...
  return <div className="space-y-8">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{data.map((a,i)=><motion.div key={a.symbol} initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} transition={{delay:i*.025}}><Link href={`/asset/${encodeURIComponent(a.symbol)}`}><Card className="glass-hover h-full p-5"><div className="flex items-start justify-between"><div className="min-w-0"><p className="text-[11px] text-[var(--muted)]">{a.symbol} · {a.exchange}</p><h2 className="mt-1 truncate text-lg font-bold">{a.name}</h2><p className="mt-1 truncate text-xs text-[var(--muted)]">{a.sector}</p></div><ArrowUpRight size={17} className="shrink-0 text-[var(--muted)]"/></div><div className="mt-8 flex items-end justify-between"><div><p className="text-xs text-[var(--muted)]">{a.price==null?"Open for analysis":"Last price"}</p><p className="mt-1 text-2xl font-black">{a.price==null?"—":`${a.currency==="USD"?"$":"₹"}${Number(a.price).toLocaleString("en-IN",{maximumFractionDigits:2})}`}</p></div>{a.personalFit!=null&&<ScoreRing value={a.personalFit} size={62} label="fit"/>}</div>{a.riskScore!=null&&<div className="mt-6 grid grid-cols-2 gap-3 border-t border-[var(--line)] pt-4 text-xs"><span className="text-[var(--muted)]">Risk <b className="ml-1 text-white">{a.riskScore}</b></span><span className="text-[var(--muted)]">Hype <b className="ml-1 text-white">{a.hypeScore}</b></span></div>}</Card></Link></motion.div>)}</div>
