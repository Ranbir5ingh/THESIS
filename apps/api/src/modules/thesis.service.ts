...
export class ThesisService {
  constructor(
    @Inject(DB) private readonly pool: Pool | null,
    private readonly market: MarketService,
  ) {}
  private requireDb() {
    if (!this.pool)
      throw new ServiceUnavailableException(
        "Supabase is not configured. Add DATABASE_URL to the API environment.",
      );
    return this.pool;
  }
  ...
  async challenge(input: {
    ...
    if (!profile)
      throw new ServiceUnavailableException(
        "Complete Investor DNA before challenging a thesis.",
      );
    ...
    try {
      ...
      console.error("[THESIS][Gemini] generateContent failed:", message);

      throw new ServiceUnavailableException(
        "THESIS could not reach Gemini. Please retry shortly.",
      );
    }
    let result: any;
    try {
      ...
      throw new ServiceUnavailableException(
        "THESIS received an invalid AI response. Please retry.",
      );
    }
    if (
      typeof result.thesisStrength !== "number" ||
      !Array.isArray(result.supportingEvidence) ||
      !Array.isArray(result.contradictingEvidence) ||
      !Array.isArray(result.biases) ||
      !["CONSIDER", "WAIT", "RETHINK"].includes(result.decision)
    )
      throw new ServiceUnavailableException(
        "THESIS received an incomplete AI response. Please retry.",
      );
    return result;
  }
  ...
  async save(input: any) {
    ...
    const assetRow = await query<{ id: string }>(
      pool,
      `INSERT INTO assets(symbol,exchange,name,sector,asset_type) VALUES($1,$2,$3,$4,$5) ON CONFLICT(symbol) DO UPDATE SET exchange=EXCLUDED.exchange,name=EXCLUDED.name,sector=EXCLUDED.sector,asset_type=EXCLUDED.asset_type RETURNING id`,
      [asset.symbol, asset.exchange, asset.name, asset.sector, asset.assetType],
    );
    await query(
      pool,
      `INSERT INTO theses(user_id,asset_id,thesis_text,thesis_score,bias_score,decision,analysis_json) VALUES($1,$2,$3,$4,$5,$6,$7)`,
      [
        input.userId,
        assetRow.rows[0].id,
        input.thesis,
        input.analysis?.thesisStrength ?? null,
        input.analysis?.biases?.[0]?.score ?? null,
        input.analysis?.decision ?? "WAIT",
  ...
  async list(userId: string) {
    ...
    return (
      await query(
        pool,
        `SELECT t.id,t.thesis_text,t.thesis_score,t.bias_score,t.decision,t.analysis_json,t.created_at,a.symbol,a.name FROM theses t JOIN assets a ON a.id=t.asset_id WHERE t.user_id=$1 ORDER BY t.created_at DESC`,
        [userId],
      )
    ).rows;
  }
